// src/setup/provider-tiers.ts — Model tier mappings for all providers.
// Extracted from default-config.ts to stay under the 200-line limit.

/** All provider → model-tier mappings for the generated config. */
export function buildProviderTiers(): Record<string, any> {
    return {
        openrouter: {
            fast: "google/gemini-2.0-flash-001",
            balanced: "minimax/minimax-m2.5",
            powerful: "anthropic/claude-sonnet-4-5",
            vision: "google/gemini-2.0-flash-001",
            summarizer: "google/gemini-2.0-flash-001",
        },
        openai: {
            fast: "gpt-4o-mini",
            balanced: "gpt-4o",
            powerful: "o3",
            vision: "gpt-4o",
            summarizer: "gpt-4o-mini",
        },
        anthropic: {
            fast: "claude-haiku-4-5",
            balanced: "claude-sonnet-4-5",
            powerful: "claude-opus-4-5",
            vision: "claude-sonnet-4-5",
            summarizer: "claude-haiku-4-5",
        },
        google: {
            fast: "gemini-2.0-flash",
            balanced: "gemini-2.5-pro",
            powerful: "gemini-2.5-pro",
            vision: "gemini-2.0-flash",
            summarizer: "gemini-2.0-flash",
        },
        xai: {
            fast: "grok-3-mini-fast",
            balanced: "grok-3",
            powerful: "grok-3",
            vision: "grok-3",
            summarizer: "grok-3-mini-fast",
        },
        vercel: {
            fast: "openai:gpt-4o-mini",
            balanced: "openai:gpt-4o",
            powerful: "anthropic:claude-sonnet-4-5",
            vision: "openai:gpt-4o",
            summarizer: "openai:gpt-4o-mini",
        },
        replicate: {
            fast: "meta/meta-llama-3-8b-instruct",
            balanced: "meta/meta-llama-3.1-405b-instruct",
            powerful: "meta/meta-llama-3.1-405b-instruct",
            vision: "",
            summarizer: "meta/meta-llama-3-8b-instruct",
        },
        huggingface: {
            fast: "meta-llama/Llama-3.2-3B-Instruct",
            balanced: "meta-llama/Llama-3.3-70B-Instruct",
            powerful: "meta-llama/Llama-3.3-70B-Instruct",
            vision: "",
            summarizer: "meta-llama/Llama-3.2-3B-Instruct",
        },
        deepseek: {
            fast: "deepseek-chat",
            balanced: "deepseek-chat",
            powerful: "deepseek-reasoner",
            vision: "",
            summarizer: "deepseek-chat",
        },
        perplexity: {
            fast: "sonar",
            balanced: "sonar-pro",
            powerful: "sonar-pro",
            vision: "",
            summarizer: "sonar",
        },
        groq: {
            fast: "llama-3.1-8b-instant",
            balanced: "llama-3.3-70b-versatile",
            powerful: "llama-3.3-70b-versatile",
            vision: "llama-3.2-90b-vision-preview",
            summarizer: "llama-3.1-8b-instant",
        },
        mistral: {
            fast: "mistral-small-latest",
            balanced: "mistral-large-latest",
            powerful: "mistral-large-latest",
            vision: "pixtral-large-latest",
            summarizer: "mistral-small-latest",
        },
        together: {
            fast: "meta-llama/Llama-3.2-3B-Instruct-Turbo",
            balanced: "meta-llama/Llama-3.3-70B-Instruct-Turbo",
            powerful: "deepseek-ai/DeepSeek-R1",
            vision: "meta-llama/Llama-3.2-90B-Vision-Instruct-Turbo",
            summarizer: "meta-llama/Llama-3.2-3B-Instruct-Turbo",
        },
        fireworks: {
            fast: "accounts/fireworks/models/llama-v3p1-8b-instruct",
            balanced: "accounts/fireworks/models/llama-v3p3-70b-instruct",
            powerful: "accounts/fireworks/models/deepseek-r1",
            vision: "accounts/fireworks/models/llama-v3p2-90b-vision-instruct",
            summarizer: "accounts/fireworks/models/llama-v3p1-8b-instruct",
        },
        deepinfra: {
            fast: "meta-llama/Llama-3.2-3B-Instruct",
            balanced: "meta-llama/Llama-3.3-70B-Instruct",
            powerful: "deepseek-ai/DeepSeek-R1",
            vision: "meta-llama/Llama-3.2-90B-Vision-Instruct",
            summarizer: "meta-llama/Llama-3.2-3B-Instruct",
        },
        cerebras: {
            fast: "llama-3.3-70b",
            balanced: "llama-3.3-70b",
            powerful: "llama-3.3-70b",
            vision: "",
            summarizer: "llama-3.3-70b",
        },
        cohere: {
            fast: "command-r",
            balanced: "command-r-plus",
            powerful: "command-r-plus",
            vision: "",
            summarizer: "command-r",
        },
        ollama: {
            fast: "llama3.2:3b",
            balanced: "llama3.3:70b",
            powerful: "llama3.3:70b",
            vision: "llava:13b",
            summarizer: "llama3.2:3b",
        },
        lmstudio: {
            fast: "qwen2.5-7b",
            balanced: "deepseek-r1:14b",
            powerful: "deepseek-r1:14b",
            vision: "",
            summarizer: "qwen2.5-7b",
        },
        baseten: {
            fast: "llama-3.1-8b-instruct",
            balanced: "llama-3.3-70b-instruct",
            powerful: "deepseek-r1",
            vision: "",
            summarizer: "llama-3.1-8b-instruct",
        },
        browserai: {
            fast: "phi-3-mini",
            balanced: "llama-3.2-3b",
            powerful: "llama-3.1-8b",
            vision: "",
            summarizer: "phi-3-mini",
        },
        aigateway: {
            fast: "google/gemini-2.0-flash",
            balanced: "anthropic/claude-sonnet-4.6",
            powerful: "anthropic/claude-opus-4.6",
            vision: "google/gemini-2.0-flash",
            summarizer: "google/gemini-2.0-flash",
        },
    };
}
